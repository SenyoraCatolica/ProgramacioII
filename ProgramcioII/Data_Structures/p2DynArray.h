...
public:
	...
	unsigned int removeWastedMemory()
	{
		...
		unsigned int diff = mem_capacity - num_elements;
	...
	bool InsertArray(unsigned int position, p2DynArray& array)
	{
		if (mem_capacity < num_elements + array.num_elements)
			Alloc(num_elements + array.num_elements);
	...
	void Prefix(p2List<TYPE>& list)
	{
		const p2List<TYPE>::node* item = list.front_node();
